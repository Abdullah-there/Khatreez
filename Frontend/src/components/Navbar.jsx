    ...
    <div className='navbar' id='navbar'>
      <nav id='main-nav' className='nav-main'>
        ...
        <ul className='nav' id="nav-ul">
          <li><NavLink to="/" onClick={handleClicked}>Home</NavLink></li>
          <li><NavLink to="/articles" onClick={handleClicked}>Articles</NavLink></li>
          <li><NavLink to="/contact" onClick={handleClicked}>Contact Us</NavLink></li>
        </ul>
    ...
    {searchValue !== "" && result.length > 0 
      ? result.map((article) => (
        <React.Fragment key={article.id}>
          ...
          ><NavLink to={`/articles/${article.id}`}>{article.title}</NavLink></li>
          <hr />
        </React.Fragment>
      ))
      : searchValue !== "" && result.length === 0 && (
        <li className='navlink-items'>No article found</li>
      )}
  </ul>
)}
